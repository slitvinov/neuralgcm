...
def _allgather_matmul_twoway(
    ...
    accum, rhs_fwd, rhs_bwd = lax.fori_loop(1, axis_size // 2,
                                            collective_matmul,
...
def _parse_einsum_subscripts(subscripts: str) -> tuple[str, str, str]:
    if '...' in subscripts:
        raise ValueError(
            f'ellipsis not supported by sharded_einsum: {subscripts}')
...
def _determine_reduce_subscript(
    ...
    for subscript in lhs_subscripts:
        if (subscript not in out_subscripts and subscript in rhs_subscripts
...
def _determine_transfer_subscript(
    ...
    subscripts = []
    for subscript in lhs_subscripts:
        if (subscript not in rhs_subscripts and subscript in out_subscripts
...
def sharded_einsum(
    ...
    *,
    ...
    transfer_subscript = _determine_transfer_subscript(*all_subscripts,
                                                       out_spec)
    reduce_axis_name = rhs_spec[rhs_subscripts.index(reduce_subscript)]
    if gather_inputs is None:
        out_shape = jax.eval_shape(functools.partial(jnp.einsum, subscripts),
                                   lhs, rhs).shape
    ...
    if gather_inputs:
        ...
        matmul_impl = functools.partial(_allgather_matmul_twoway,
        ...
        matmul_impl = functools.partial(_matmul_reducescatter_twoway,
